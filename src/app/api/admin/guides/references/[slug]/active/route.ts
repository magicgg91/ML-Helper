...
import { guideReferenceSlugs } from "@/lib/admin-tools";
...
export async function PATCH(request: Request, { params }: RouteContext<"/api/admin/guides/references/[slug]/active">) {
  ...
  const { slug } = await params; if (!(guideReferenceSlugs as readonly string[]).includes(slug)) return NextResponse.json({ error: "reference_not_found" }, { status: 404 });
