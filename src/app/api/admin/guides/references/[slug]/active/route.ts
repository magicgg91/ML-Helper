...
export async function PATCH(
  request: Request,
  { params }: RouteContext<"/api/admin/guides/references/[slug]/active">,
) {
  ...
  // Bloc 33/G: "templiers" has its own independent Calculator row now (not
  // the "templars" tool's row), so it's a normal member of
  // referenceToolSlugs like the other 3 — no special-casing needed here.
  const { slug } = await params;
  if (!(referenceToolSlugs as readonly string[]).includes(slug))
  ...
  const reference = await prisma.$transaction(async (tx) => {
    ...
    await tx.auditLog.create({
      data: {
        ...
        ...auditMessageColumns(
          auditMessage(
            `reference.${parsed.data.active ? "activate" : "deactivate"}`,
            { actor: session.user.name ?? session.user.id, slug },
          ),
        ),
