...
import { CityParametersEditor, TemplarParametersEditor } from "@/components/named-parameters-editor";
import { RankingAdminEditor } from "@/components/ranking-admin-editor";
import { getCityParameters, getTemplarParameters } from "@/lib/admin-formulas-server";
...
export default async function EditToolPage({ params }: PageProps<"/admin/tools/[id]">) {
  ...
  let title: string;
  if (id === "city-parameters") { title = t("city-parameters"); content = <CityParametersEditor initial={await getCityParameters()} />; }
  else if (id === "ranking") { title = t("ranking-editor"); content = <RankingAdminEditor initialConfig={await getRankingConfig()} />; }
  ...
  return <main className="admin-main"><AdminBackLink href="/admin/tools" /><h1>{title}</h1>{content}</main>;
}
