-- Replace only the untouched legacy template. Content already customized by an
-- administrator is deliberately preserved.
...
WHERE "key" = 'legal_notice'
  AND instr(json_extract("content", '$.fr'), '[Nom ou raison sociale de l’éditeur]') > 0;

-- Backfill an installation where a Super Admin already exists but the content
-- row is absent. Fresh installations are seeded transactionally during setup.
INSERT OR IGNORE INTO "static_content" ("id", "key", "content", "updated_at", "updated_by")
SELECT lower(hex(randomblob(16))), 'legal_notice', json_object('fr', '# Mentions légales
...
## Droit applicable
Les présentes mentions légales sont soumises au droit français.'), CURRENT_TIMESTAMP, "id"
FROM "users"
