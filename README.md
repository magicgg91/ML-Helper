...
| `TRACKING_ORIGIN` | No                              | Origin of the visit-tracking script configured in Admin → Configuration, for example `https://stats.example.com`. Only needed when that script is served from another domain: its own requests are governed by the CSP's `connect-src`, which otherwise allows this site only. The script URL itself stays editable in the admin — this names the origin its measurements may be sent to. |
