...
### Bloc 0 — Restructuration de navigation, décidée
...
### Bloc 1 — Bug actif, à corriger en premier
...
### Bloc 2 — Fondation : internationalisation
*À faire avant tout le reste pour éviter de retoucher deux fois chaque écran (une fois pour le contenu, une fois pour l'i18n).*
...
- **Aucune ligue sélectionnée par défaut, partout** *(point 23)* — Paramètres du joueur, Classement, Gemmes (Optimisation + Budget), Simulateur de Stuff, Comparateur de stuff, Troupes attaque démo, Level Up : tous les sélecteurs de ligue doivent démarrer vides (placeholder "— Choisir —"), avec repli propre côté calcul tant qu'aucune ligue n'est choisie. **Exception (Classement, Troupes attaque démo, Level Up)** : ces 3 sélecteurs s'alignent automatiquement sur la ligue du joueur dès qu'elle est définie — **y compris au chargement initial si déjà en cache (localStorage)**. Si le sélecteur dépendant a déjà une valeur choisie manuellement, elle n'est pas écrasée.
...
- **30 lignes de valeurs manquantes** *(point 16)* — Équipements de Combat, 10 sets Commun/Rare/Épique (section 7.1).
