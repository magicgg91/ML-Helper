# Archive — ML-Helper : Setup/Phases 0-2 + Blocs 0 à 59

**Ce fichier est une archive historique**, extraite de `brief-demarrage-codex.md` le 03/09/2026 pour alléger ce dernier (devenu trop volumineux après 81 blocs). **Tout le contenu ci-dessous est ✅ terminé et mergé** — conservé uniquement pour la traçabilité complète, pas pour un usage actif.

**Pour le travail en cours, se référer à `brief-demarrage-codex.md`**, qui contient : l'état d'avancement à jour, les rappels transverses actifs, les blocs à partir du 60, les chantiers en réserve non cadrés, et les gros chantiers en attente.

**Le cahier des charges complet (`cahier-des-charges-ml-helper.md`) reste la seule source de vérité fonctionnelle** — ce fichier ne contient que l'historique du découpage en blocs et leur statut de livraison, pas les spécifications elles-mêmes (qui restent à jour et complètes dans le cahier des charges).
...
## Blocs 0 à 59 — détail complet (tous terminés et mergés)
...
- **Combat — Taux de gain d'XP** *(point 31)* — ✅ **Construit et livré** (formule + seuils confirmés, UI conforme au prototype : un champ VP, mode attaquant/cible, sortie en tableau des 5 paliers avec plage de VP adverse — voir section 7.1 du cahier des charges). Éditeur admin fermé au Bloc 23 (PR #43).
- **Combat — Troupes max envoyées en "attaque démo"** *(point 32)* — ✅ **Construit et livré** (formule confirmée : `TroupesMax = X% × Remparts(niveau_ville_visée)`, X selon la ligue de l'attaquant : Bronze 100%, Argent 50%, Or/Platine 40%, Diamant/Légende 30%. Vitesse d'attaque réduite = information seulement, pas intégrée au calcul). Éditeur admin fermé au Bloc 23 (PR #43).

**5b — Référentiel (section Référentiels de `/guides`, PAS Outils) :**
- **Level Up (référentiel, pas calculateur)** *(inclus dans le point 18)* — ✅ **Construit et livré**, accessible sur `/referentiels/level-up` (racine changée depuis `/guides/referentiels/*`, voir Bloc 50). Formule troupes ✅ verrouillée pour **Légende, Diamant et Bronze (identique pour les trois)**, ✅ verrouillée séparément pour **Platine** (formule distincte), ✅ verrouillée séparément pour **Or** (formule distincte, ne pas confondre avec Légende malgré une ressemblance trompeuse aux petits niveaux — voir piège méthodologique en section 7.1 du cdc), ✅ **formule d'XP requis par niveau verrouillée** (universelle, identique sur les 6 ligues, régression exacte sur 110 points), cycle de coffres tous les 10 niveaux ✅ **confirmé universel sur les 6 ligues** (Coffre→Urne→Coffret à bijoux→Jarre→Caisse, boucle de 5, identique partout — seules les formules de troupes varient par ligue) — contenu exact des coffres couvert par le guide Level Up plutôt que la donnée structurée. **Reste : données troupes Argent, seule donnée manquante** (champ admin dédié ajouté au Bloc 42 — éditable, en attente de la valeur réelle).
...
### Bloc 54 — Liens croisés : lien manquant Combat/Expédition + bandeau agrandi ✅ **Terminé** (PR #76)
*Nouveau. Indépendant du reste.*

- **A** — 🐛 Lien référentiel→outil carrément absent pour Équipement de Combat et Équipement d'Expédition (existe pourtant outil→référentiel). Ajouter le lien manquant, réciprocité complète comme pour Gemmes/Templiers/Level Up.
- **B** — Refonte du bandeau de lien croisé (Bloc 53), jugé trop petit : bouton plus grand, phrase intégrée à l'intérieur du bouton (pas au-dessus), image (`.cross-reference-thumb`) à 5rem.

### Bloc 55 — Position du bandeau de lien croisé sur les outils Compétences ✅ **Terminé** (PR #77, 782 tests)
*Notes de livraison : les 3 points vérifiés en navigateur réel (position du bandeau mesurée, alignement bouton/titre confirmé, badges Langues visuellement corrects).*
*Nouveau. Indépendant du reste.*

- 🐛 Le bandeau (Simulateur de Stuff, Simulateur d'Équipement d'Expédition, Gemmes, Templiers) est actuellement en haut de page — doit être en bas, après le contenu de l'outil. Corriger sur les 4 outils de la catégorie Compétences.
- **Admin Guides** : bouton "Nouveau" repositionné à côté du titre "Contenu éditorial" (au lieu de sur la ligne de filtre).
- **Admin Guides** : nouvelle colonne indiquant les langues dans lesquelles chaque guide est réellement écrit (badges/icônes par langue) — utile pour repérer les guides à traduire.
...
- **🐛 Constat définitif (2 tests confirmés)** : le HTML brut (`<img>`) ne fonctionne jamais côté public (tableau ou paragraphe) ; la syntaxe markdown pure `![alt](url)` fonctionne mais sans contrôle de taille possible (rendu trop grand dans les 2 cas testés). → **Voir Bloc 56 ci-dessous.**

### Bloc 56 — 🐛 Support du HTML brut côté public (rendu markdown) ✅ **Terminé** (PR #77, 785 tests)
*Note de livraison : cause confirmée — `rehype-raw` manquant côté public, l'éditeur admin l'ajoute en interne. Corrigé à la racine dans `markdown-plugins.ts`, partagé par Boutique/guides/mentions légales — couvre les 3 d'un coup, pas un correctif isolé.*
*Nouveau. Indépendant du reste.*

- Le rendu public de `@uiw/react-md-editor` ne supporte pas le HTML brut (`<img>` a minima), contrairement à l'aperçu admin — cause probable : absence d'un plugin type `rehype-raw` sur le pipeline public. Corriger pour que le public affiche le HTML comme l'admin. Nécessaire pour contrôler la taille des images de contenu éditorial (impossible en markdown pur).
- Vérifier si guides et mentions légales sont affectés par le même écart (même éditeur) — corriger à la racine si le pipeline est partagé, plutôt que cas par cas.

### Bloc 57 — 🐛 Logs admin : double entrée + nom obsolète (référentiel Boutique) ✅ **Terminé** (PR #78, 785 tests)
*Notes de livraison : A résolu au niveau backend (endpoint transactionnel combiné, pas juste un patch d'affichage) — 1 clic = 1 requête = 1 ligne de log. B — plus aucune occurrence de "Consommables" vérifiée sur l'ensemble des messages de log dynamiques de ce référentiel. 2 trouvailles de review Codex traitées avant merge.*
*Nouveau. Indépendant du reste.*

- Une seule sauvegarde (bouton unique, Bloc 42) génère 2 lignes de log distinctes ("modifié le texte d'introduction du référentiel Consommables" + "modifié le référentiel Boutique") au lieu d'une seule. Corriger pour qu'une sauvegarde unique produise une seule ligne.
- Nom obsolète "Consommables" toujours présent dans un des deux messages (renommé "Boutique" depuis le Bloc 48) — corriger, et vérifier l'ensemble des messages de log touchant ce référentiel, pas seulement ce cas précis.

### Bloc 58 — 🚨 Révision majeure : zone intro Boutique en tableau structuré, plus de texte libre ✅ **Terminé** (PR #79, 795 tests)
*Notes de livraison : A vérifié en navigateur réel (CRUD admin + rendu public + comportement "jamais filtré", captures à l'appui), correctif `rehype-raw` du Bloc 56 laissé intact pour guides/mentions légales. **Trouvaille de review Codex traitée** : `saveReferenceTable` manquait d'atomicité (lecture/upsert/log non transactionnels) — corrigé avec `prisma.$transaction`, durcit toutes les routes de référentiels partageant ce helper, pas seulement Boutique.*
*Nouveau. Indépendant du reste. Suite au feuilleton HTML/markdown/float des Blocs 55-57 : simplification radicale plutôt que continuer à corriger un système fragile.*

- La zone de texte libre markdown est **supprimée entièrement** — plus de HTML, plus de float, plus de rendu public/admin divergent.
- Remplacée par un **tableau "Intro"**, même pattern que les 4 tableaux de catégorie (Conseillers/Équipement/Expédition/Inventaire) : **3 colonnes — Image, Nom, Description**.
- **Toujours affiché en 1er bloc**, avant les filtres de catégorie, et **jamais affecté par les filtres** (ne se masque jamais, contrairement aux 4 tableaux de catégorie).
- **Même CRUD complet** que les 4 autres tableaux (ajout icône "+", suppression croix rouge + confirmation, édition) — réutilise directement le même composant, pas de nouveau développement.
- Contenu (2 lignes prévues : Saphirs, Inventaire) à resaisir par le joueur dans le nouveau tableau une fois construit — pas une reprise automatique du texte markdown existant.
- Retirer le titre de colonne "Image" dans les tableaux affichés côté public (tableau Intro et les 4 tableaux de catégorie) — l'image reste affichée, seul le libellé d'en-tête disparaît.

### Bloc 59 — 🚨 Faille de sécurité : droits d'accès à l'historique admin mal appliqués ✅ **Terminé** (PR #79, 795 tests)
*Nouveau, retour testeur — vraie faille de sécurité, ampleur relativisée : seul `admin` a un problème, pas les autres rôles. Indépendant du reste, priorité haute.*
*Note de livraison : correctif à 2 lignes sur la matrice de capacités partagée — chaque guard de page, guard API et lien de nav en dérive déjà, donc un seul point de correction a suffi à tout aligner (page + API + nav + dashboard "dernières actions").*

- 🐛 `admin` a actuellement accès complet (lecture + purge) à `/admin/logs`, alors que la purge doit être réservée à `super_admin` uniquement.
- **`admin`** : lecture seule sur cette page — aucune action de purge visible ni accessible (UI ET API, pas juste le bouton masqué).
- **`guides_manager`/`calculators_manager`/`references_manager`** : déjà correctement sans accès, ne pas casser en corrigeant `admin`.
- **`read_only`** : périmètre précis — lecture seule sur **Tableau de bord (sans la section "dernières actions"), Outils, Référentiels, Guides uniquement**. Aucun accès à Historique ni Utilisateurs (ni lien nav, ni accès direct URL).
