...
~~Les traductions DE/ES/TR déjà faites pour le namespace `admin.*` (texte fixe de l'interface admin) restent dans les fichiers de traduction (harmless, pas besoin de les retirer) mais deviennent simplement inutilisées une fois `AdminLocaleToggle` limité à 2 options.~~ **✅ Livré (Bloc 118) — révision : ces traductions inutilisées sont retirées.** Le Bloc 116/C l'avait fait pour le seul journal d'audit ; le Bloc 118 l'étend à tout ce que l'admin affiche pour lui-même — les namespaces `admin`, `login` et `roles` ne sont plus présents que dans `en.json` et `fr.json` (1 320 clés supprimées de `de/es/tr.json`). La règle de parité générale (mêmes clés dans les 5 locales, Bloc 93/E2) exclut désormais ces 3 namespaces et reste stricte partout ailleurs ; `adminNamespaces` (`src/lib/translations.ts`) est la liste de référence, recalculée depuis le graphe d'imports par `src/i18n/admin-locale-scope.test.ts` pour qu'un futur namespace admin ne puisse pas repartir en 5 langues sans être vu. **La distinction ci-dessus est inchangée** : les sélecteurs de langue par contenu éditorial gardent les 5 langues, et l'admin continue de gérer les 5 langues publiques (activation/désactivation dans `/admin/config`) — c'est l'interface admin elle-même, pas ce qu'elle publie, qui est en EN/FR.
...
- **🚨 Cohérence linguistique — architecture précisée, exigence renforcée.** Chaque texte visible dans l'UI, **sans aucune exception, public ET admin**, doit être référencé par une **clé de traduction**, jamais de texte codé en dur dans une langue quelconque. Deux mécanismes distincts, chacun devant permettre d'ajouter une langue **sans aucune modification de code** :
  - **Texte d'interface statique** (labels, boutons, menus, messages d'erreur/confirmation, tout `/admin/*` inclus) → fichiers de traduction **JSON**, un fichier par langue (ex: `en.json`, `fr.json`, `es.json`...), structure de clés identique entre tous les fichiers. Ajouter une langue = ajouter un nouveau fichier JSON traduit, zéro ligne de code à toucher.
    - **⚠️ Précision (Bloc 118) — la règle « clés identiques entre tous les fichiers » vaut pour le texte public ; le texte propre à l'interface admin n'existe qu'en EN/FR.** Le mécanisme ci-dessus est inchangé pour l'admin : pas une ligne de texte codée en dur, tout passe par une clé. C'est la *liste des langues* qui diffère, parce que `src/proxy.ts` ramène toute requête `/admin` et `/login` à EN ou FR depuis le Bloc 47/C — une traduction DE/ES/TR de ces libellés ne pourrait jamais s'afficher. Les namespaces concernés sont listés dans `adminNamespaces` (`src/lib/translations.ts`) : `admin`, `login`, `roles`. **Ajouter une langue publique n'est pas alourdi pour autant** : le nouveau fichier ne porte que les namespaces publics. `src/i18n/admin-locale-scope.test.ts` vérifie les deux moitiés de la règle — parité stricte entre les 5 langues côté public, EN/FR seulement côté admin — et recalcule la liste des namespaces admin depuis le graphe d'imports, pour qu'un futur namespace admin ne reparte pas en 5 langues sans être vu.
    - **✅ Livré (Bloc 120) — la promesse « zéro ligne de code » est désormais tenue.** Elle ne l'était pas : `launchLocales` était une constante éditée à la main dont dépendait le routing. Elle est maintenant **dérivée du contenu de `messages/`** au moment du build (`scripts/generate-launch-locales.ts`, exécuté avant `dev`, `build`, `test` et après `install`), et déposer `messages/<locale>.json` suffit à faire apparaître la langue partout — routing, préfixe d'URL, `hreflang`, `og:locale`, sélecteurs de contenu éditorial, redirections héritées et tableau d'activation de `/admin/config`. Le scan ne peut pas se faire au runtime : `src/lib/translations.ts` est importé par des composants client, où Turbopack refuse `node:fs` (build en échec, vérifié). Seuls restent décidés en code la langue par défaut et le socle EN/FR toujours actif — des choix produit, pas des conséquences des fichiers présents. **Le nouveau fichier doit en revanche ne pas porter les namespaces `admin`/`login`/`roles`** (règle du Bloc 118 ci-dessus), ce qu'un test signale.
